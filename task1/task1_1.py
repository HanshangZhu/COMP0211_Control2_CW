...
# Convert from physical to system
...
for k in range(num_steps):
    ...
    cur_y = motor_model.step(V_a)   # current y(output) at time t
    x_cur = motor_model.getCurrentState()   # x(t+1)
    
    # Output measurement (Terminal Voltage)
    V_terminal[k] = cur_y
    
    # predict x(t+1)_hat according to input and current output
