...
def main():
    ...
    Qcoeff_joint_pos = [10000] * num_controls
    ...
    plt.subplot(3, 1, 1)
    Color = ['blue','orange','red','green','yellow','purple','black']
    for i in range(num_joints):
        plt.plot(time_all, q_mes_all[:, i], color=Color[i],label=f'Joint {i+1}')
    for i in range(num_joints):
        plt.plot(time_all, np.ones_like(time_all)*goal_joints[i], color=Color[i],linestyle='--', label=f'Joint {i+1} Ref')
